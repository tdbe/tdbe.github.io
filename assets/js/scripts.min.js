...
window.onload = function (){
	...
	const elList = document.querySelectorAll('.giscus_widget_index');
	for(let i = 0; i < elList.length; i++){
		const item = elList.item(i);
		//console.log("found: "+item);
		//const elInner = el.contentDocument.querySelector('div');
		item.shadowRoot.querySelector('iframe').removeAttribute("scrolling");
	}
}
...
function handleIsMobile(){
	if(isMobile())
	{
		const mobiles = document.querySelectorAll(".hidableMobile");
		for (let i = 0; i < mobiles.length; i++) {
			mobiles.item(i).classList.remove("hide");
		}
		
		const desktops = document.querySelectorAll(".hidableDesktop");
		for (let i = 0; i < desktops.length; i++) {
			desktops.item(i).classList.add("hide");
		}
	}
	else
	{
		const desktops = document.querySelectorAll(".hidableDesktop");
		for (let i = 0; i < desktops.length; i++) {
			desktops.item(i).classList.remove("hide");
		}
	}
...
function doOnResize(){
	...
	OnResizeEnd = setTimeout(handleResize, 100);
}

function handleResize(){
	var width = Math.min( $(window).width(), window.innerWidth);
	//em:
	var smPd = 2.5; 
	var lgPd = 8.08;
	var liq;
	
	//var smallWidth = 'width=640, initial-scale=1.0';
	//var largeWidth = 'width=device-width, initial-scale=1.0';
	//var smallWidth = 'width=640, initial-scale=1.0 zoom 100% target-densitydpi=high-dpi';
	var smallWidth = 'width='+width*0.5+', initial-scale=1.0 zoom 100% target-densitydpi=high-dpi';
	var largeWidth = 'width='+width*0.5+', initial-scale=1.0 zoom 100% target-densitydpi=high-dpi';
	...
	if(width <= 640){
		...
		/*
		var initScale = width / 640;
		$('#myViewport').attr({ 
			'content':'width=640, initial-scale='+initScale
		});
		*/
		...
		/*
		$('#post-index #main .hentry').css({ 
			"padding-left": smPd+"em!important",
			"padding-right": smPd+"em!important"
		});
		...
		$('#post .entry-content').css({ 
			"padding-left": smPd+"em!important",
			"padding-right": smPd+"em!important"
		});
		*/
		
		$('#post-index #main .hentry').attr({ 
			"style" : "padding-left:"+ smPd+"em!important;"+
			" padding-right:"+ smPd+"em!important;"
		});
		
		$('#page .entry-content').attr({ 
			"style" : "padding-left:"+ smPd+"em!important;"+
			" padding-right:"+ smPd+"em!important;"
		});
		
		$('#post .entry-content').attr({ 
			"style" : "padding-left:"+ smPd+"em!important;"+
			" padding-right:"+ smPd+"em!important;"
		});
		...
		//856 == 9
		//640 == 4
		liq = (width - 640)*(lgPd-smPd)/(856-640)+smPd; 
		liq *= 1.1; 
		...
		$('#post-index #main .hentry').attr({ 
			"style" : "padding-left:"+ liq+"em!important;"+
			" padding-right:"+ liq+"em!important;"
		});
		...
		/*
		$('#post-index #main .hentry').css({ 
			"padding-left": lgPd+"em!important",
			"padding-right": lgPd+"em!important"
		});
		...
		*/
		...
		$('#post-index #main .hentry').attr({ 
			"style" : "padding-left:"+ lgPd+"em!important;"+
			" padding-right:"+ lgPd+"em!important;"
		});
...
function(a) {
    ...
    a.fn.fitVids = function(b) {
        ...
        return d.className = "fit-vids-style", d.innerHTML = "&shy;<style>               .fluid-width-video-wrapper {                 width: calc(100% + 40px);	margin-left: -20px;	margin-right: -20px;                              position: relative;                       padding: 0;                            }                                                                                   .fluid-width-video-wrapper iframe,        .fluid-width-video-wrapper object,        .fluid-width-video-wrapper embed {           position: absolute;                       top: 0;                                   left: 0;                                  width: 100%;                              height: 100%;      border-radius:8px;                    }                                       </style>", e.parentNode.insertBefore(d, e), b && a.extend(c, b), this.each(function() {
    ...
    function(a, b, c) {
        function d(a, c) {
            var d = b.createElement("social").style,
                e = "webkit Moz o ms".split(" ");
            if (c in d) return !0;
            for (var f = 0, g = e.length; g > f; f++)
                if (e[f] + a in d) return !0;
            return !1
        }
        ...
        var g, h, i = {
            isGradeA: "querySelectorAll" in b && !a.blackberry && !("ontouchstart" in window) && ("undefined" == typeof window.navigator.msMaxTouchPoints || 0 === window.navigator.msMaxTouchPoints),
            minCount: 1,
            ...
            getCounts: function(a, b) {
                var d, e, g, j = i.selectors,
                    k = i.cache,
                    l = {};
                for (g in j) d = a.find(j[g]), e = d.find("." + i.classes.countContent), e.length ? l[g] = e : (l[g] = h.clone(), d.append(l[g]));
                return k[b] || (k[b] = c.ajax({
                    url: f() + i.serviceUrl,
                    data: {
                        url: b
                    },
                    ...
                    for (var b in a) a.hasOwnProperty(b) && l[b] && a[b] > i.minCount && l[b].addClass(i.classes.minCount).html(i.normalizeCount(a[b]))
                }), k[b]
            },
            init: function(a) {
                var b = i.getFacebookAction(a),
                    c = [b],
                    d = i.isSmallSize(a),
                    e = i.getUrl(a),
                    f = i.plugins.init,
                    g = i.isCountsEnabled(a);
                ...
                for (var h = 0, j = f.length; j > h; h++) f[h].call(a);
            ...
            bindEvents: function(a, d, e, f) {
                ...
                for (var k = i.plugins.bind, l = 0, m = k.length; m > l; l++) k[l].call(a, h, d, f)
            }
        };
        c(function() {
